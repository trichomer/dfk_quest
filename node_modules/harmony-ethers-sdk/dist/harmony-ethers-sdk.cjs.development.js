...
var HarmonyAddress = /*#__PURE__*/function () {
  ...
  _proto.getBasic = function getBasic(addr) {
    if (address.isAddress(addr)) {
      return address.getAddress(addr).substring(2);
    }

    if (isBech32Address(addr) || isBech32TestNetAddress(addr)) {
      var _bech32$decode = bech32.bech32.decode(addr),
          prefix = _bech32$decode.prefix,
          words = _bech32$decode.words;

      if (prefix === HRP || prefix === tHRP) {
        return address.getAddress(bytes.hexlify(bech32.bech32.fromWords(words))).substring(2);
      }
    }
  ...
  _createClass(HarmonyAddress, [{
    key: "basicHex",
    get: function get() {
      return bytes.hexlify(this.basic);
    }
  }, {
    key: "checksum",
    get: function get() {
      return address.getAddress(this.basic);
    }
  }, {
    key: "bech32",
    get: function get() {
      return bech32.bech32.encode(HRP, bech32.bech32.toWords(bytes.arrayify(this.basic)));
    }
  }, {
    key: "bech32TestNet",
    get: function get() {
      return bech32.bech32.encode(tHRP, bech32.bech32.toWords(bytes.arrayify(this.basic)));
    }
  }]);
...
var logger = /*#__PURE__*/new logger$2.Logger("hmy_transaction/0.0.1");
...
function formatNumber(value, name) {
  var result = bytes.stripZeros(bignumber.BigNumber.from(value).toHexString());
...
function formatDecimal(value) {
  if (typeof value === "string") {
    return [units.parseUnits(value, 18).toHexString()];
  }

  return [bignumber.BigNumber.from(value).toHexString()];
}
...
function formatMsg(type, value) {
  switch (type) {
    case exports.Directive.CreateValidator:
      {
        ...
        return [getAddress(msg.validatorAddress).checksum, formatDescription(msg.description), formatComissionRates(msg.commissionRates), formatNumber(msg.minSelfDelegation, "minSelfDelegation"), formatNumber(msg.maxTotalDelegation, "maxTotalDelegation"), msg.slotPubKeys.map(function (key) {
          return bytes.arrayify(key);
        }), (_msg$slotKeySigs = msg.slotKeySigs) == null ? void 0 : _msg$slotKeySigs.map(function (sig) {
          return bytes.arrayify(sig);
        }), formatNumber(msg.amount, "amount")];
      }

    case exports.Directive.EditValidator:
      {
        var _msg = value;
        return [getAddress(_msg.validatorAddress).checksum, _msg.description ? formatDescription(_msg.description) : [], _msg.commissionRate ? formatDecimal(_msg.commissionRate) : "0x", _msg.minSelfDelegation ? formatNumber(_msg.minSelfDelegation, "minSelfDelegation") : "0x", _msg.maxTotalDelegation ? formatNumber(_msg.maxTotalDelegation, "maxTotalDelegation") : "0x", _msg.slotKeyToRemove ? bytes.hexlify(_msg.slotKeyToRemove) : "0x", _msg.slotKeyToAdd ? bytes.hexlify(_msg.slotKeyToAdd) : "0x", _msg.slotKeySig ? bytes.hexlify(_msg.slotKeySig) : "0x", _msg.active != null ? _msg.active ? constants.One.toHexString() : constants.Two.toHexString() : constants.Zero.toHexString()];
      }

    case exports.Directive.Delegate:
    case exports.Directive.Undelegate:
      {
        var _msg2 = value;
    ...
    case exports.Directive.CollectRewards:
      {
        var _msg3 = value;
    ...
    default:
      logger.throwArgumentError("invalid type", "type", bytes.hexlify(type));
  }
}
...
function serializeTransaction(transaction, signature) {
  ...
  transactionFields.forEach(function (fieldInfo) {
    ...
    value = bytes.arrayify(bytes.hexlify(value, options)); // Fixed-width field
    ...
    fields.push(bytes.hexlify(value));
  });
  return encodeTransaction(transaction, fields, signature);
}

function serializeStakingTransaction(transaction, signature) {
  var fields = [transaction.type === 0 ? "0x" : bignumber.BigNumber.from(transaction.type).toHexString(), formatMsg(transaction.type, transaction.msg), formatNumber(transaction.nonce || 0, "nonce"), formatNumber(transaction.gasPrice || 0, "gasPrice"), formatNumber(transaction.gasLimit || 0, "gasLimit")];
  return encodeTransaction(transaction, fields, signature);
}
...
function encodeTransaction(transaction, fields, signature) {
  ...
  } else if (signature && !bytes.isBytesLike(signature) && signature.v && signature.v > 28) {
    // No chainId provided, but the signature is signing with EIP-155; derive chainId
    chainId = Math.floor((signature.v - 35) / 2);
  }

  fields.push(bytes.hexlify(chainId)); // @TODO: hexValue?
  ...
  fields.pop();
  v += chainId * 2 + 8;
  fields.push(bytes.hexlify(v));
  fields.push(bytes.stripZeros(bytes.arrayify(sig.r) || []));
  fields.push(bytes.stripZeros(bytes.arrayify(sig.s) || []));
  return RLP.encode(fields);
}
...
function handleText(value) {
  return Buffer.from(bytes.arrayify(value)).toString();
}
...
function handleActive(value) {
  var status = bignumber.BigNumber.from(value);

  if (status.eq(constants.Zero)) {
    return null;
  }

  if (status.eq(constants.One)) {
    return true;
  }

  if (status.eq(constants.Two)) {
    return false;
  }
...
function handleStakingTransaction(transaction) {
  ...
  try {
    tx.v = bignumber.BigNumber.from(transaction[5]).toNumber();
  } catch (error) {
    return tx;
  }

  tx.r = bytes.hexZeroPad(transaction[6], 32);
  tx.s = bytes.hexZeroPad(transaction[7], 32);
  ...
  if (bignumber.BigNumber.from(tx.r).isZero() && bignumber.BigNumber.from(tx.s).isZero()) {
    ...
    var digest = keccak256.keccak256(RLP.encode(raw));

    try {
      tx.from = transactions.recoverAddress(digest, {
        r: bytes.hexlify(tx.r),
        s: bytes.hexlify(tx.s),
        recoveryParam: recoveryParam
      });
    } catch (error) {}

    tx.hash = keccak256.keccak256(RLP.encode(transaction));
  }
...
function handleTransaction(transaction) {
  ...
  try {
    tx.v = bignumber.BigNumber.from(transaction[8]).toNumber();
  } catch (error) {
    return tx;
  }

  tx.r = bytes.hexZeroPad(transaction[9], 32);
  tx.s = bytes.hexZeroPad(transaction[10], 32);
...
function parseTransaction(payload) {
  return handleTransaction(RLP.decode(bytes.arrayify(payload)));
}
function parseStakingTransaction(payload) {
  return handleStakingTransaction(RLP.decode(bytes.arrayify(payload)));
}
function parse(rawTransaction) {
  var payload = bytes.arrayify(rawTransaction);
  var transaction = RLP.decode(payload);
...
var HarmonyFormatter = /*#__PURE__*/function (_Formatter) {
  ...
  _proto.getDefaultFormats = function getDefaultFormats() {
    ...
    formats.transaction.toShardID = providers.Formatter.allowNull(number);
    formats.receipt.type = providers.Formatter.allowNull(transactionType);
    Object.assign(formats.block, {
      nonce: number,
      epoch: bigNumber,
      viewID: number,
      mixHash: hash,
      stakingTransactions: providers.Formatter.allowNull(providers.Formatter.arrayOf(hash))
    });
    formats.stakingTransaction = {
      hash: hash,
      type: transactionType,
      blockHash: providers.Formatter.allowNull(hash, null),
      blockNumber: providers.Formatter.allowNull(number, null),
      transactionIndex: providers.Formatter.allowNull(number, null),
      confirmations: providers.Formatter.allowNull(number, null),
      gasPrice: bigNumber,
      gasLimit: bigNumber,
      nonce: number,
      r: providers.Formatter.allowNull(this.uint256.bind(this)),
      s: providers.Formatter.allowNull(this.uint256.bind(this)),
      v: providers.Formatter.allowNull(number),
      raw: providers.Formatter.allowNull(data)
    };
    Object.assign(formats.blockWithTransactions, {
      ...
      viewID: number,
      mixHash: hash,
      stakingTransactions: providers.Formatter.allowNull(providers.Formatter.arrayOf(this.stakingTransactionResponse.bind(this)))
    });
    formats.cXReceipt = {
      blockHash: hash,
      blockNumber: number,
    ...
    formats.createValidatorMsg = {
      ...
      slotPubKeys: providers.Formatter.arrayOf(value)
    };
    formats.createValidatorRequestMsg = properties.shallowCopy(formats.createValidatorMsg);
    formats.createValidatorRequestMsg.slotKeySigs = providers.Formatter.arrayOf(value);
    formats.editValidatorMsg = {
      validatorAddress: address,
      commissionRate: providers.Formatter.allowNull(decimal, "0x0"),
      minSelfDelegation: providers.Formatter.allowNull(bigNumber, "0x0"),
      maxTotalDelegation: providers.Formatter.allowNull(bigNumber, "0x0"),
      slotPubKeyToAdd: providers.Formatter.allowNull(value, null),
      slotPubKeyToRemove: providers.Formatter.allowNull(value, null)
    };
    formats.editValidatorRequestMsg = properties.shallowCopy(formats.editValidatorMsg);
    formats.editValidatorRequestMsg.slotKeySigs = providers.Formatter.allowNull(providers.Formatter.arrayOf(value), []);
    formats.editValidatorRequestMsg.active = providers.Formatter.allowNull(function (value) {
      return value;
    }, null);
    formats.delegateMsg = {
    ...
    formats.delegation = {
      ...
      amount: value,
      reward: value,
      undelegations: providers.Formatter.allowNull(providers.Formatter.arrayOf(function (undelegation) {
        return providers.Formatter.check(formats.undelegation, {
  ...
  _proto.decimal = function decimal(value) {
    ...
    return units.parseUnits(value, 18);
  };
  ...
  _proto.msg = function msg(type, value) {
    switch (type) {
      case exports.Directive.CreateValidator:
        {
          var msg = providers.Formatter.check(this.formats.createValidatorMsg, value);
          msg.commissionRates = providers.Formatter.check(this.formats.commissionRate, {
            rate: units.formatUnits(bignumber.BigNumber.from(value.commissionRate), 18),
            maxRate: units.formatUnits(bignumber.BigNumber.from(value.maxCommissionRate), 18),
            maxChangeRate: units.formatUnits(bignumber.BigNumber.from(value.maxChangeRate), 18)
          });
          msg.description = providers.Formatter.check(this.formats.description, value);
          return msg;
        }

      case exports.Directive.EditValidator:
        {
          var _msg = providers.Formatter.check(this.formats.editValidatorMsg, value);

          _msg.description = providers.Formatter.check(this.formats.description, value);
          return _msg;
        }

      case exports.Directive.Delegate:
        return providers.Formatter.check(this.formats.delegateMsg, value);

      case exports.Directive.Undelegate:
        return providers.Formatter.check(this.formats.undelegateMsg, value);

      case exports.Directive.CollectRewards:
        return providers.Formatter.check(this.formats.collectRewardsMsg, value);
  ...
  _proto.transactionResponse = function transactionResponse(transaction) {
    ...
    var result = providers.Formatter.check(this.formats.transaction, transaction); // 0x0000... should actually be empty STRING
  ...
  _proto.stakingTransactionResponse = function stakingTransactionResponse(transaction) {
    ...
    var result = providers.Formatter.check(this.formats.stakingTransaction, transaction);
  ...
  _proto.receipt = function receipt(value) {
    ...
    var result = providers.Formatter.check(this.formats.receipt, value);
    return result;
  };

  _proto.cXReceipt = function cXReceipt(value) {
    return providers.Formatter.check(this.formats.cXReceipt, value);
  };

  _proto.delegation = function delegation(value) {
    return providers.Formatter.check(this.formats.delegation, {
...
var logger$1 = /*#__PURE__*/new logger$2.Logger("hmy_provider/0.0.1");
...
var ApiHarmonyProvider = /*#__PURE__*/function (_JsonRpcProvider) {
  ...
  function ApiHarmonyProvider(url) {
    ...
    _this._nextId = crypto.randomBytes(1).readUInt8();
    return _this;
  }
        ...
        while (1) {
          switch (_context.prev = _context.next) {
            ...
            case 29:
              ...
              getNetwork = properties.getStatic(this.constructor, "getNetwork");
              _context.prev = 31;
              return _context.abrupt("return", getNetwork(bignumber.BigNumber.from(chainId).toNumber(), shardingStructure));

            case 35:
              _context.prev = 35;
              _context.t3 = _context["catch"](31);
              return _context.abrupt("return", logger$1.throwError("could not detect network", logger$2.Logger.errors.NETWORK_ERROR, {
                chainId: chainId,
        ...
        while (1) {
          switch (_context15.prev = _context15.next) {
            ...
            case 4:
              ...
              return _context15.abrupt("return", web.poll( /*#__PURE__*/_asyncToGenerator( /*#__PURE__*/runtime_1.mark(function _callee14() {
        ...
        while (1) {
          switch (_context17.prev = _context17.next) {
            ...
            case 2:
              ...
              return _context17.abrupt("return", web.poll( /*#__PURE__*/_asyncToGenerator( /*#__PURE__*/runtime_1.mark(function _callee16() {
                var result;
...
exports.ApiHarmonyProvider = ApiHarmonyProvider;
exports.HRP = HRP;
...
exports.parse = parse;
exports.parseStakingTransaction = parseStakingTransaction;
exports.parseTransaction = parseTransaction;
exports.serialize = serialize;
exports.serializeStakingTransaction = serializeStakingTransaction;
exports.tHRP = tHRP;
//# sourceMappingURL=harmony-ethers-sdk.cjs.development.js.map
